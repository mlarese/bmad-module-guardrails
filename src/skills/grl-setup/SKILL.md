...
- **Roster** → `{project-root}/_bmad/custom/config.toml`, una tabella `[agents.grl-agent-*]`
  per figura. È il passo che porta le nove figure nel party mode: `resolve_party.py` costruisce
...
Devono comparire tutte e nove le chiavi `grl-agent-*` accanto agli agenti già installati. Se
...
**non** la tabella degli agenti. Su un'installazione YAML le nove figure vanno quindi registrate
...
- **Non crea gruppi di party mode.** Le nove figure stanno nella stanza principale insieme ai
...
1. Mostra cosa è stato scritto: le nove figure registrate (nome, icona, titolo), il valore di
...
3. **Proponi `grl-profile` e, se l'utente accetta, eseguilo subito.** È il passo che rende utile
   ...
   quanto saranno severe tutte e nove le figure. Se rifiuta, va bene: digli che ogni figura
